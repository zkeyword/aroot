...
class media{
	...
	public function addMedia($author, $title, $description, $content){
		$db = $this->db;
				
		if( !$this->checkMedia($title) ){
			$array = array(
				'author'      => $author,
				'title'       => $title,
	...
	public function modifyMedia($id, $title, $description, $content){
		if( !$id ) exit();
		$db          = $this->db;
		$array       = array(
			'title'       => $title,
	 ...
	 * @param {boolean} 
	 */
	public function checkMedia($title){
		$db  = $this->db;
	...
	public function mediaList($current = 1,  $pageSize = 10){
		$db = $this->db;
		$find      = 'p.post_id id,
					  p.title title,
		...
		$link      = ' p.author = u.id AND p.type = 3 AND p.post_id = m.post_id ';
		$limit     = 'LIMIT '. ($current - 1) * $pageSize .','. $pageSize;
		$condition = 'WHERE' . $link;
		$allSql    = 'SELECT '. $find .' FROM '. table('post') .' p, '. table('user') .' u,'. table('postMeta') .' m '. $condition;
